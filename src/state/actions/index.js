...
// ###############################################
export const GET_CURRENT_TOPIC_SUCCESS = 'GET_CURRENT_TOPIC_SUCCESS';
export const GET_CURRENT_TOPIC_FAIL = 'GET_CURRENT_TOPIC_FAIL';
// ###############################################
...
export const setBearerToken = token => {
  return dispatch => {
    dispatch({
      type: SET_BEARER_TOKEN_SUCCESS,
      payload: { error: '', token: token },
    });
    if (token == null) {
      dispatch({
        type: SET_BEARER_TOKEN_FAIL,
...
export const getCurrentTopic = () => {
  return dispatch => {
    if (!store.getState().currentTopic) {
      dispatch({
        type: GET_CURRENT_TOPIC_FAIL,
        error: 'No Current Topic',
      });
    } else {
      dispatch({
        type: GET_CURRENT_TOPIC_SUCCESS,
        error: '',
      });
      return store.getState().currentTopic;
    }
  };
};
  ...
  return dispatch => {
    ...
    axiosWithAuth()
      ...
      .then(res => {
        ...
        }); // what is this setting into state?
      })
      .catch(err => {
        dispatch({
          type: CREATE_NEW_TOPIC_FAIL,
          payload: { error: err, isFetching: false },
        });
      });
  };
...
export const getContexts = () => {
  return dispatch => {
    ...
    axiosWithAuth()
      ...
      .then(res => {
        dispatch({
          type: GET_CONTEXTS_SUCCESS,
          payload: { error: '', isFetching: false, contexts: res.data },
        });
      })
      .catch(err => {
        dispatch({
          type: GET_CONTEXTS_FAIL,
          payload: { error: err, isFetching: false },
        });
      });
  };
...
export const setTopic = topic => {
  return dispatch => {
    ...
    if (!topic) {
      dispatch({
        type: SET_TOPIC_FAIL,
        payload: { error: 'Bad topic passed ( check your args )' },
      });
    }
  };
};
  ...
  return dispatch => {
    ...
    if (!survey) {
      dispatch({
        ...
        payload: { error: 'Bad survey passed ( check your args )' },
      });
    }
  };
};
